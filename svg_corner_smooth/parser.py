...
_GENERATED_OVERLAY_IDS = {
    "detected-corners-overlay",
    "arc-preview-overlay",
    "diagnostics-overlay",
    "rejected-corners",
    "__corner_highlight__",
}
...
def _strip_generated_overlays(root: ET.Element) -> int:
    """
    Remove overlay/debug groups generated by previous tool runs.

    Users frequently re-upload processed SVG outputs. Stripping these known
    overlay groups keeps analysis/rounding focused on the original artwork.
    """
    removed = 0
    for parent in list(root.iter()):
        for child in list(parent):
            child_id = child.get("id")
            if child_id in _GENERATED_OVERLAY_IDS:
                parent.remove(child)
                removed += 1
    return removed
...
def parse_svg_document(source: str | bytes | ET.ElementTree, debug: bool = False, strict: bool = False) -> ParsedSvgDocument:
    ...
    removed_overlays = _strip_generated_overlays(root)
    if debug and removed_overlays > 0:
        print(f"[debug] Removed {removed_overlays} generated overlay group(s) from input SVG.")
